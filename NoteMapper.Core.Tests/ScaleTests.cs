...
namespace NoteMapper.Core.Tests
{
    public static class ScaleTests
    {
        ...
        public static string Major(int noteIndex, string key)
        {
            Scale scale = Scale.Parse(noteIndex, key);
            return string.Join(",", scale.Select(x => x.GetName(AccidentalType.Sharp)));
        }

        [TestCase(0, "Minor", ExpectedResult = "C,D,D#,F,G,G#,A#")]        
        public static string Minor(int noteIndex, string key)
        {
            Scale scale = Scale.Parse(noteIndex, key);
            return string.Join(",", scale.Select(x => x.GetName(AccidentalType.Sharp)));
        }
        ...
        public static string NotesBetween(int noteIndex, string key)
        {
            Scale scale = Scale.Parse(noteIndex, key);
